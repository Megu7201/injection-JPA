node{
    stage("Git clone"){
        git url:"http://192.168.11.11:3000/andy/InjectionTest.git"
    }
        ...
        withSonarQubeEnv('sonar'){
            withMaven(maven:'maven'){
                ...
                withCredentials([usernamePassword(credentialsId:'92da05f6-e4ed-4bb0-a317-1df36d74ff66',usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')]){
                    sh "mvn sonar:sonar  -D sonar.login=$USERNAME -D sonar.password=$PASSWORD -D sonar.host.url=http:\\192.168.11.11:9000 -D sonar.projectKey=injection"
                }
            }
        }
    }
